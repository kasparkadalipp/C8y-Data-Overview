...
function getTooltip(d){
    ...
    if (d.dataCount != null) {
        data["Event count"] = d.dataCount.toLocaleString();
    }
    ...
    if(d.group === 1){
        data['Unique event types'] = uniqueFields
    }
    if(d.group === 2){
        data['Unique fragments'] = uniqueFields
    }
    if(d.group === 3 && uniqueFields > 0){
        data['Unique nested keys'] = uniqueFields
    }
...
function mouseoverNode(mouseEvent, d) {
    ...
    hoverTimeout = setTimeout(() => {
        ...
        const tooltipData = getTooltip(d);
        if (tooltipData.size === 0) {
...
const uniqueGroups = [...new Set(data.nodes.map(node => node.group))].filter(group => group in groupDescriptions).sort();
