...
const simulation = d3.forceSimulation(data.nodes)         // Force algorithm is applied to data.nodes
    ...
    .force("center", d3.forceCenter(...screenCenter).strength(.4))     // This force attracts nodes to the center of the svg area
    // .force("collide", d3.forceCollide(50).strength(.5))
    .force("collide", rectangleCollide())
    .force("radical", d3.forceRadial(d => groupForceRadial[d.group], ...screenCenter).strength(1))
    // .force("radical", d3.forceRadial(d => d.group ** 1.7 * 300, ...screenCenter).strength(.4))
    // .force("radical2", d3.forceRadial(d => d.group ** 1.5 * 300, ...screenCenter).strength(.4))
    .on("tick", ticked);
...
function getTooltip(d){
    const data = {
        ...
        // "Measurement count": d.measurementCount
    };
...
function updateTooltipPosition(event) {
    tooltip
        .style("left", `${event.pageX + 10}px`)  // Position tooltip to the right of the mouse cursor
        .style("top", `${event.pageY + 10}px`);  // Position tooltip below the mouse cursor
}
...
//                     if (Math.abs(x) < wi + wj && Math.abs(y) < hi + hj) {
//                         var overlapX = ((wi + wj) - Math.abs(x)) * strength,
//                             overlapY = ((hi + hj) - Math.abs(y)) * strength;
    
//                         // Apply a simple resolution strategy: push apart along the axis of least overlap
//                         if (overlapX < overlapY) {
//                             node.vx += x > 0 ? overlapX : -overlapX;
//                             data.vx -= x > 0 ? overlapX : -overlapX;
//                         } else {
//                             node.vy += y > 0 ? overlapY : -overlapY;
//                             data.vy -= y > 0 ? overlapY : -overlapY;
//                         }
//                     }
//                 }
//                 return;
//             }
//             // Update bounds check to account for rectangle dimensions
//             return x0 > xi + wi || x1 < xi - wi || y0 > yi + hi || y1 < yi - hi;
//         }
//     }
    
//     function prepare(quad) {
//         if (quad.data) {
//             quad.width = quad.data.width;
//             quad.height = quad.data.height;
//             return;
//         }
//         quad.width = quad.height = 0;
//         for (var i = 0; i < 4; ++i) {
//             if (quad[i]) {
